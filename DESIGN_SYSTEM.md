...
| Dock | `shell.css` `.dock` (+ `.cook-dock`/`.gro-dock`/`.wk-decide` for per-screen padding) | The screen's one action, sticky above the ask bar. Built as `.cook-dock` for the cook journey and generalised on 2026-09-10 when nav rule 2 put the same strip on Plan and Shop. Quiet secondaries ride along as `.dock-link` text; a second fill would break Rule 5. A screen with no single action has no dock. Cook's root got one on 2026-09-13 (`.cook-root-dock`, "Start cooking" / "Mark eaten"; nothing on a night with no cook). |
