...
| Step head + back link | `shell.css` `.wk-head`/`.wk-title`/`.wk-sub`/`.wk-back`, `.gro-head`/`.gro-sub`/`.gro-back` | A tab with steps (Meals, Grocery) opens each step with a title and one line, and a named "‹ Parent" link — never `history.back()`, never a new page. Added 2026-09-08. |
