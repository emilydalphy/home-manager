...
| Step head + crumb | `shell.css` `.wk-head`/`.wk-title`/`.wk-sub`, `.gro-head`/`.gro-sub`, and `.crumb` for the back link | A tab with steps (Plan, Shop, Cook) opens each step with a title and one line, and a named "‹ Parent" crumb — never `history.back()`, never a new page. Added 2026-09-08. The back link was three near-identical classes (`.wk-back`, `.gro-back`, `.cook-focus-back`) until 2026-09-10; it is one `.crumb` now, with `.on-spruce` for the variant inside Cook's hero. |
