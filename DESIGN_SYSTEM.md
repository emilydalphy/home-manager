...
| The menu (What we're eating) | `shell.css` `.wk-menu`, `.wk-menu-card`, `.wk-menu-head`; `wkMenuGroups`/`wkMenuRowHtml`/`wkMenuHtml` in `shell.js` | The draft's front door (Emily's decision D, 2026-09-15; again 2026-09-21): one card per meal type, one `.wk-row` per dish with the days it covers and one fact ("Mon, Wed · Mexican, as asked" — `asked` from `get_week_menu`) and the reason after it, Swap on every row through the same swap sheet as the day cards (`data-wk-day-index` names the dish's first day ahead), and Tweak where the Plan row (1A) rule allows. No day notes, no steppers. |
