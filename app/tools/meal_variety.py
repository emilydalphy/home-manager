...
def _cap_at(caps: dict | None, date: str, slot: str) -> int | None:
    ...
    Today the caller hands {date: minutes or None} (dinner caps only);
    a slot-aware helper is on its way (weekday fresh lunches get their own
    cap), keyed {(date, slot): minutes or None}. Both shapes answer here,
    so nothing else in this module has to know which one it was given.
    """
    if not caps:
        return None
