...
def apply_prep_day_batches(weekly_plan_id: int) -> dict:
    ...
    if prep_days:
        ...
        for item in cook_ahead_repeats(weekly_plan_id):
            # A day the household un-batched cooks fresh, so it leaves the
            # dish's leftover days BEFORE they are cut into three-day
            # batches (both rules, merged 2026-09-24).
            later = [d for d in item["later"] if d["entry_id"] not in no_batch_days]
            if not later:
                declined.append({"kind": "dish", "dish": item["dish"]})
                continue
            for first, covered in _within_three_days(dict(item, later=later)):
                if not covered:
                    continue
                result = set_cook_ahead(first["entry_id"], [d["entry_id"] for d in covered])
