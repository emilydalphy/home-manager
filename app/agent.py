...
quicker instead, and don't ask whether they mean takeout. It never needs anything changed \
...
`said` back as the one line. The one exception: status 'needs_confirmation' means the night \
the dish would move onto is already marked cooked and nothing has changed — ask its `message` \
and wait; pass confirm_cooked only once they say yes.
...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "description": "\"We're going out tonight\" / \"we'll grab something\" / \"nobody's cooking tonight\" — settle tonight in one call, with no follow-up question. Takeout, leftovers or cereal all mean the same thing here, so don't ask which. Tonight's dish moves to the next free night of the plan when there is one (`moved_to` says which), otherwise it comes off the week and `use_soon` names anything already bought for it that won't keep — say that back in one line if it isn't empty. Either way the night ends deliberately empty: never call it a gap, never offer to fill it, never describe the week as short because of it. `already` true means nothing changed because the night was already empty — `already_reason` 'night_off' is a night off somebody already took (say so plainly, don't take it again), 'away' is a night nobody was home for anyway (say THAT, never \"night off\"). It never asks for anything to be changed first: a dish cooked double for a later night, with no free night, is cooked on the first night it was feeding instead (same size, `moved_to` says which) and tonight's share goes in the freezer; a leftovers night, or a dinner already cooked, goes in the freezer (`frozen` names it). `said` is the line to say back. A status of 'refused' means there is no plan covering that night and nothing changed — say `message` back rather than retrying. A status of 'needs_confirmation' means NOTHING changed yet: the night the dish would move onto is already marked cooked, and moving onto it replaces that record — ask the household `message` in plain words and stop; only if they say yes in this conversation, call again with confirm_cooked true. Pass `day` only for a day other than today. For a trip or a night nobody is home at all, use set_slot_need or set_away_stretch instead — those are about who's in the house, this is about not cooking.",
        "input_schema": {
            "type": "object",
            "properties": {
                "day": {"type": "string", "description": "YYYY-MM-DD. Omit for tonight, which is almost always what's meant."},
                "confirm_cooked": {"type": "boolean", "description": "Defaults to false. The household's explicit yes to moving the dish onto a night already marked cooked — never set this on your own initiative and never on the first call. Only after a needs_confirmation answer, and only if they said yes."},
            },
            "required": [],
        },
    },
