...
- **2026-09-21 — Three tests went red under a far-date pin, and all three
  ...
  - **Re-measured on the merge base first, and the table still holds.** The
    ...
    records "3 at 2026-11-20"; it is **0**, and **the figure was wrong when it
    was written rather than overtaken** — the first version of this entry said
    `main` had moved under it, which is a second wrong explanation in place of
    the first and is exactly the class this log keeps having to unpick.
    Verified by checking the tree out at the commit that wrote it: these three
    files read **171 passed** at `--today=2026-11-20` on `0de8aff`, on
    `29bc929` and on `f0f7238` alike, while Jan-15 reads 3 and Jul-04 reads 2
    at all three. So nothing moved: the other figures were right all along and
    that one never described these files.
  ...
  - **`test_inventory_two_places_pick.py` — the app's rule held at every step
    ...
    `mark_defrost_asked` already carries. **The reason first given here — that
    a household seed's skew is the same size as the one-day margin — is
    WRONG, and a reviewer showed it by running it rather than arguing it:**
    `household_date(-N)` passes at every pin and under a Kiritimati straddle
    too, because the skew moves both seeds equally and a bare date always
    sorts before a same-day datetime. The deviation stands on the better
    reason. `datetime('now', '-N days')` is the same clock, the same units
    and the same string format the app's own writer stamps that column with,
    so the seed sits behind those writes by construction rather than by a
  ...
  - **`test_grocery_spices.py` — "long enough ago" was written as a date, and
    ...
    did shrink and it is worth the line:** a fixed 2026-01-01 stood about 263
    days clear of the 56-day cadence, and `_days_ago(120)` stands 64 clear.
    Ample, and it would go for an unrelated reason if `RECENTLY_BOUGHT_DAYS`
    ever rose past 120.
  - **`test_food_quality_floor.py` — a SEPARATE PROCESS is a clock nothing
    ...
    report then asked its own unfrozen `datetime('now')` and opened its window
    at **`2026-09-14`** — SEVEN days back, not the one `--days 1` asks for,
    because `collect()` reads this feed as
    `get_recent_plan_quality(days=max(days, 7))`. The first version of this
    entry and the comment in the test both said `2026-09-20`, which is a
    figure a later session would try to re-derive and could not; the row is
    eight months outside the window either way, so the conclusion is
    untouched. It truthfully printed "Nothing broke."
    `@pytest.mark.live_clock`, which is exactly what that exemption is for —
    and it works because `sqlite_clock.install` is depth-counted, so
    the inner pin re-points SQLite at the real instant and hands the
    session's own pin back afterwards.
  - **ONE OF THEM WAS INVERTED, NOT MERELY BLIND, AND THAT IS THE WORST
    STATE A TEST CAN BE IN.** At `--today=2026-01-15`, on `main`,
    `test_add_then_use_agree_on_one_row` **passed when the 2026-09-13 bug it
    exists to catch was reintroduced**, and failed when the app was correct.
    Measured, not inferred:

    | `main`, `--today=2026-01-15`, that one test | result |
    |---|---|
    | unmutated (app correct) | **1 failed** |
    | `ORDER BY` dropped — the historical bug | **1 PASSED** |

    The first version of this entry said "failed whether the app was right or
    wrong", which is true of the `updated_at ASC` mutation and undersells
    this one. **Bounded honestly, because that fixture was never the only
    thing holding the rule up:** the FILE still caught the dropped `ORDER BY`
    at every pin on both fixtures (5 red), since its other tests seed the
    FRIDGE row as the recent one. And on this branch that single test still
    PASSES under that mutation — with no `ORDER BY` SQLite answers in rowid
    order, which is the pantry row this test already means to land on, so it
    genuinely cannot tell the two apart. "Every mutation bites" is therefore
    a claim about the FILE and never about that test.
  - **ALL THREE had zero discriminating power at that pin, not two** — the
    spice is the third: on `main` at `--today=2026-01-15` it reads `1 failed`
    unmutated AND under both of its own mutations. After the re-seed all three
    read `1 passed` unmutated and redden under the rule they are named for.
  - **Eight mutations, and these are FILE-LEVEL counts throughout** — one
    convention, said once, because the first version of this entry mixed
    file-level inventory numbers with per-test spice numbers inside a single
    sentence. Each bites at a backward pin, a forward pin AND unpinned: the
    inventory `ORDER BY` dropped (the original 2026-09-13 bug, **5** red),
    `updated_at ASC` (**6**), the `id DESC` tie-break reversed (**1**),
    `_bought_lately` always False (**2**), `_due` always False (**3**), the
    FOOD line never printed (**1**), a food finding flipping the exit code
    (**1**), and a food finding printed under BROKEN (**1**).
  - **Structural rather than tuned to one pin, which is the stronger claim
    and was not made at first.** The three files read **171 passed** at
    `--today=2025-12-31`, `2024-03-15` and `2020-06-01`, where the merge base
    is **3 failed** at each.
  ...
  - **Found on the way and NOT fixed — its own card, and it is bigger than
    ...
    ask the app about a week the household is not in. `straddle` is a blocking
    CI job, so this is not latent. **It presents as a FLAKY job rather than a
    broken one, which is why nobody has chased it:** Niue is UTC-11 against
    Toronto's UTC-4, so it is a different DAY only while Toronto reads
    00:00-06:59 — seven hours in twenty-four, green the other seventeen.
    Measured both sides of that edge: the same ten files under the same TZ
    read **13 failed / 335 passed** inside the window and **1 failed / 347
...
- **2026-09-18 — "Already have" measured its week from midnight UTC, and read
  ...
  - **THE BOUNDARY IS THE HALF WITH TEETH, and it is not the half the test
    ...
    `datetime('now')`, from all SEVEN writers of it — `grocery.py` :1345,
    :1379, :1504, :1848 and :1914, `pre_shop.py` :278, `staples.py` :779,
    counted 2026-09-18; the four that set it back to NULL are un-writers
    and are not among them — and the cutoff was a calendar DATE, so
    the string comparison opens the window at **midnight UTC**. West of UTC
    that is merely generous: Toronto's window opened four hours early in
    EDT and five in EST — measured, the band is the evening before the
    period, 20:00 to midnight local, 19:00 to midnight between November and
    March.
  ...
  - **WHICH CLOCK, stated plainly: the household's DAY, compared as a UTC
    ...
    write side is right and is left alone (seven writers, rows already on disk,
  ...
  - **`cooker.household_zone()` is EXTRACTED from `household_now`, not written
    beside it.** This log records three copies of the UTC→household
    conversion (cooker, digest, tonight) and calls folding them into one its
    own card — **and the real count is FOUR**: `holidays._answered_on`
    (:955-968) does the same read of `households.timezone`, the same
    `ZoneInfo`, the same Toronto fallback and the same `.astimezone`, and no
    entry here had named it. So a fifth, inline here, would have been the
    wrong direction.
  ...
  - **ONE BEHAVIOUR CHANGE EVERY EXISTING HOUSEHOLD SEES — AND IT IS A
    PRODUCT CALL FOR EMILY, NOT SOMETHING A CLOCK TICKET GETS TO DECIDE.**
    A decision made before the period began is no longer listed: four hours
    before in EDT, five in EST, i.e. the evening before the period starts,
    from 20:00 local and from 19:00 between November and March. The first
    version of this entry called that "the fix working" and stopped there.
    It IS the fix working — the window is the period — and it is not only
    that, because of WHEN those hours fall and WHAT the screen is for.
    - **`planning_anchor` defaults to `sunday_before`**, so a household that
      never answered the weekday picker plans on Sunday for a Monday start
      (`suggest_planning_period`'s own docstring: "ready the Sunday before,
      Monday start — which is the exact old default"). The band this
      narrowing covers is therefore Sunday evening, which is exactly when
      such a household is approving the week and sorting the list these
      decisions come off.
    - **Measured end to end, not reasoned**: a Toronto household, an
      approved Mon–Sun plan, and one real `drop_grocery_item_pre_shop` at
      Sunday 21:00 local — both clocks frozen together at that instant, so
      the stamp is SQLite's own (`2026-09-14 01:00:00` UTC).

      | read at | main | this branch |
      |---|---|---|
      | Sunday 21:30 local | `['rice']` | `['rice']` |
      | Monday 09:00 local — the shop | `['rice']` | `[]` |
      | Wednesday 18:00 local | `['rice']` | `[]` |

      The two agree on the Sunday for opposite reasons, which is worth
      knowing: main is inside the period because the SERVER has already
      rolled into Monday, and this branch is outside it and falls back to
      the Monday-before cutoff. The row goes the moment the period it was
      made for actually starts — the morning they shop.
    - **And there is nowhere else to look.** `get_already_have_decisions`
      has exactly one caller, `/api/grocery-list/already-have-summary`, and
      that is the only SCREEN anywhere that shows a `status='removed'` row.
      Nothing can put one back except `undo_pre_shop_drop`, which is not a
      chat tool — nor is `get_already_have_decisions`. (Be precise about the
      other half: `list_grocery_list(status='all')` IS a chat tool and DOES
      include a removed row, so "they cannot see it" is too strong. They
      cannot UNDO it.) So what falls out of the window is not a line on a
      screen, it is the household's ability to change their mind — from the
      morning of the shop onward, for every existing household, in the hours
      they were most likely to have decided.
    - **Left exactly as built**, because the window being the period is the
      fix and widening it back is a different decision from the clock one.
      The honest lever if Emily wants those hours kept is the WINDOW, not
      the boundary — the evening before a period is arguably part of
      planning it, which is what `sunday_before` says it is. Its own card.
  ...
  - **It is 5 and not 4 because `households.timezone` is read TWICE, on two
    connections** — `household_today()` reaches it through `household_now()`
    → `household_zone()`, and then `household_zone()` is called again for the
    boundary. Harmless on local SQLite and deliberately not restructured
    here. **What is worth knowing before anybody does restructure it is that
    the two reads do not FAIL the same way:** `household_today()` swallows
    any exception, logs, and falls back to the SERVER's date, while
    `household_zone()` falls back to Toronto. So on a transient read failure
    on the first connection and a success on the second, this function puts a
    server DAY against a household BOUNDARY — half the bug it just fixed,
    arriving from inside the fix. Written down rather than closed; one read
    threaded into both is its own card.
  - **Pre-existing and not a regression, named so nobody reports it as new:**
    a BLOB in `households.timezone` raises `TypeError` out of
    `household_zone` — its `except (ZoneInfoNotFoundError, ValueError)` does
    not cover it, and `household_now` has had that same narrow clause inline
    all along. Unreachable through any app write path: `digest.py:266` is the
    only writer, its parameter is typed `str | None`, and it validates
    through `ZoneInfo` before it writes. What is NEW is the blast radius, and
    it is narrower than it first looks — **measured on both apps**: on main
    `get_already_have_decisions` answers fine (it never reads the zone) and
    `get_cooker_view` also answers fine, because every reader of that clock
    sits behind a broad `except Exception` that logs and falls back. This
    branch adds the first caller that reads `household_zone()` DIRECTLY, with
    nothing swallowing underneath it, so the Review screen's confirmation
    section is the first surface where that value becomes a 500. Cook is
  ...
  - **`.github/workflows/tests.yml` said this bug was open, and it is
    corrected in the same change that closes it** — this file's own standing
    rule. That comment named four `get_already_have_decisions` failures under
    a Monday pin east of UTC+9 as a reason never to pin the straddle matrix;
    they are gone, and every test that touches that function is green at all
    seven weekday pins under `Pacific/Kiritimati` (226 passed, each pin).
    **The INSTRUCTION itself stands, for a residue that is a different bug**
    — the four that remain in the bullet above, monday only (measured: 0 on
    the other six weekdays for both files). The comment names those two
    files now, so the next reader does not assume they are the ones it used
    to be about, and does not read "fixed" as "safe to pin".
...
- **2026-09-18 — `live_clock` handed a test a clock an hour out whenever the
  ...
  - **The far-future sweep WAS run on review, and it is not otherwise clean —
    ...
    **branch 3 failed / 5665 passed**. The branch removes exactly the two
    ...
    the app; see that entry at the top of this log. ONE of the figures above is
    wrong and always was: 2026-11-20 is **0**, not 3 — measured at `0de8aff`,
    the commit that wrote it, as well as at `29bc929` and `f0f7238`. It was
    not overtaken by `main` moving; it never described these three files. The
    other three figures hold exactly.**
...
- **2026-09-17 — A `--today` pin put the HOUSEHOLD four hours before the hour
  ...
  - **ROOT CAUSE, and it is a contradiction rather than a coverage choice.**
    ...
    `cooker.household_now()` and `digest.py:574` are
    `datetime.now(timezone.utc).astimezone(zone)`; `tonight.py:107` is
    `datetime.now(zone)`, a different call SHAPE through the same patched
    branch; and **`calendar_feed.py:804/856/902` is an affected reader the
    first version of this entry never mentioned** — write and read go through
    one call, so it was internally consistent either way, and what changes is
    that the instant it records now matches `utcnow()`, SQLite's `'now'` and
    `time.time()`. **NOT `holidays.py`**, which the first version wrongly
    named: its live clock is `date.today()` (:447, :924), with :968 converting
    a stored stamp, so it never reads the aware form at all.
  ...
  - **AND THE CONSEQUENCE OF THAT, WHICH IS THE REAL COST OF THIS CHANGE AND
    WHICH THE FIRST VERSION OF THIS ENTRY DID NOT STATE: a weekday pin stops
    naming the household's weekday off-zone.** The seam was zone-INDEPENDENT —
    the property this entry itself identifies as why it survived — so until
    now `household_today()` equalled the pinned date in every process zone.
    After this it moves with the process's zone. Measured: `TZ=Asia/Tokyo
    POMONA_TEST_TODAY=sunday` puts the process on Sunday 2026-09-20 and the
    household on **Saturday** 2026-09-19 20:00, against Sunday 05:00 on
    `main`. Whole suite there: `main` 1 failed / 5414 passed, this branch
    **0 failed / 5429 passed** — and the failure that disappears,
    `test_tonight_night_off::test_the_night_is_planned_empty_and_never_open`,
    **goes green for the wrong reason, because the household is now on
    Saturday rather than because anything was fixed.** A test passing for the
    wrong reason, produced by this change. Contained on CI (`clock` pins
    Toronto, and there is a tripwire on that line) and the new semantics are
    the honest ones — but a developer running `pytest --today=sunday` on a
    non-Toronto laptop no longer exercises Sunday from the app's point of
    view. `_parse_pin`'s docstring says so, and `household_pin(..., on=...)`
    is the way to name the household's own weekday.
  ...
  - **NOT DONE, and this is the more ambitious reading of the card:** making
    ...
    suite already forbids it. **Take the conclusion and not the number**: the
    count depends on how the mutation is written AND on the process zone.
    Mutating `_freeze_args`'s return to `local - offset + timedelta(hours=4)`
    reddens 13 at `TZ=America/Toronto` (3 of them in `test_frozen_clock`) and
    16 at `TZ=Pacific/Niue`; a reviewer's own implementation of the same idea
    reddened 9 (4 in that file). What every version breaks is the same set of
    guards: `test_a_bare_date_lands_mid_morning`,
    `test_a_pin_can_carry_a_time_of_day`, this branch's
    `test_the_pin_is_still_the_process_wall_clock`, and — in the reviewer's
    version — `test_live_clock_beats_a_pin`. "A pin is the process's local
    wall clock" is a promise a dozen files rest on. Emily's to reopen;
  ...
  - `tests/test_pin_hour_household_clock.py` (14; **9 red / 5 passed** with the
    aware fix reverted). **The first version of this entry said 10, and 10 is
    the count of CATCH-LABELLED cases rather than of red ones** — this log has
    had to unpick a red-count claim twice before and this is the third. The
    case that is labelled CATCH and cannot be red is
    `test_household_pin_names_the_households_hour_in_any_zone[UTC]`: the seam
    was the process offset applied twice and UTC's offset is zero, so the bug
    could not manifest there. Its docstring says so now, and it is kept
    because dropping it would leave the container's own zone untested here.
    **It is a mutation count and NOT a red-against-`main` count** — the first
    version gave a false reason for that ("the file needs no symbol `main`
    lacks"); line 33 imports `household_pin`, which `main`'s conftest has not
    got, so against `main` the file is an ImportError and a collection error.
    The conclusion was right and the reason was wrong.
    Six mutations run, five bite: the aware fix reverted (9 red),
    `household_pin` without its conversion (4), the naive clock shifted
    instead (see below), `household_pin` on the default zone rather than the
  ...
  - **Numbers, all read off the runs.** Unpinned **5432 passed / 0 failed**
    ...
    entry records). 5418 -> 5432 is this file's 14 exactly. Pins:
    `clock (monday/friday/saturday)` **5429 passed / 3 skipped / 0 failed**
    each, `clock (sunday)` 1 failed — the pre-existing one below.
    **WHICH ZONE STRADDLES DEPENDS ON THE HOUR, so the re-run after review
    could not use the same one**: by then Toronto read 09:31 and every zone
    west of it was back on the same date, leaving only UTC+10 and east.
    `Pacific/Auckland` (UTC+12, verified: process 2026-09-18, household
    2026-09-17) gives **3 failed / 5429 passed** — and those three are the
    documented "pin machinery above UTC+9" trio, **measured identical on
    `main` at the same zone** (3 failed / 14 passed in `test_frozen_clock`,
    both trees, same three names). They are what
    `overnight/today-pin-east-of-utc9` fixes, which the merged run below
    confirms. At
    `TZ=Pacific/Kiritimati` `test_frozen_clock` is **3 failures before and 3
    after**, the same three, which are the separate "pin machinery above
    UTC+9" card. **The first version of this entry claimed 4 -> 3 and that was
    an artifact of my own measurement**: I ran this branch's MODIFIED
    `test_frozen_clock.py` against an UNMODIFIED `conftest.py`, so the seam
    test was asserting the new behaviour under the old clock. A half-reverted
    tree is not the merge base. The seam test sets `TZ=Pacific/Niue` itself,
    so the outer zone never reached it and it passes on `main` at Kiritimati
    (run alone there: 1 passed). Nothing was made worse anywhere; nothing was
    made better there either.
  - **MERGING WITH `overnight/today-pin-east-of-utc9` — READ THIS BEFORE
    RESOLVING, BECAUSE GIT WILL NOT ASK.** That branch is already pushed and
    the two overlap on three files. Measured on a real trial merge, not
    reasoned about: **`tests/test_frozen_clock.py` and `tests/conftest.py`
    AUTO-MERGE WITH NO CONFLICT**, and `.github/workflows/tests.yml` and this
    file conflict.
    - **The clean auto-merge is the dangerous one.** Its new test
      `test_a_pin_east_of_utc_plus_9_holds_every_clock_on_one_instant` asserts
      the OLD aware behaviour (`aware.date() == datetime.date(2026, 9, 13)
      and aware.hour == 9`), so the merged tree is **1 failed / 5432 passed**
      at Toronto, that test being the failure. Apply the one-line fix
      its own message prescribes (`aware.date() == datetime.date(2026, 9, 12)
      and aware.hour == 19`) and it is **5433 passed / 0 failed** at Toronto
      AND at a verified `Pacific/Kiritimati` straddle. The two fixes compose;
      both numbers were run here.
    - **Also stale after that merge, in the cleanly-auto-merged file:**
      `pinned_utc_now()`'s docstring (conftest ~:347-352), which justifies
      `utcnow()` over `now(timezone.utc)` on the grounds that "the aware form
      comes back as the local wall time wearing a UTC label". The CODE stays
      right — `utcnow()` is what `_sqlite_now` and node's pin are built from —
      and only the reason becomes false.
    - **`tests.yml`, hunk by hunk.** Hunk 1: take east's factual update, DROP
      east's "no longer load-bearing" sentence (false after this branch), DROP
      this branch's "still fail 3 tests / Tokyo not Kiritimati" sentence
      (false after east's), KEEP this branch's load-bearing paragraph, and add
      a clause saying the TZ on the UNPINNED `pytest` job genuinely is still
      belt-and-braces — only `clock` is load-bearing. Hunk 2: this branch's
      retirement of the straddle job's reason 2, PLUS east's Monday-pin
      warning kept verbatim and renumbered; that one is a separate finding and
      is still true.
    - **THE TWO BRANCHES SAW THE SAME SEAM AND JUDGED IT DIFFERENTLY, and
      this one is right.** East-of-utc9 called it "invisible today because
      `conftest.household_today()` and `cooker.household_now()` both read the
      aware form, so they are wrong together and agree." They do agree — and
      they agree on an answer four hours (at Kiritimati, a whole day) from the
      household's real clock, which is what cost the four pinned jobs their
      evening coverage. Its entry is **superseded on that point only**;
  ...
  - **A SEPARATE BUG, found and NOT fixed: `clock (sunday)` is RED on
    ...
    **It is NOT the class `conftest.household_today()` exists for** — an
    earlier version of this bullet said so, and that would have sent the
    follow-up the wrong way. At `TZ=America/Toronto` the two clocks agree, so
    seeding off the household changes nothing; the cause is `TONIGHT = WED`
    hard-wired (`tests/test_tonight_night_off.py:44-50`) against a pin that
    lands on Sunday. Already being fixed on its own branch.
...
- **2026-09-17 — Swapping a dish onto a night that has gone by is refused,
  ...
  Loop Board bug, the third of the date-guard set after `add_dish_day`
  ("+") and `drop_dish_from_day` ("−") — **the card called it the last one
  and it is not; see "the set is incomplete" below.**
  ...
  - **WHY IT IS NOT IN THAT FUNCTION, which is the whole card, and it was
    ...
    week went with it. Silently.** An independent reviewer reproduced that
    collateral half directly and it is worse than stated here: on a week
    with TWO clashes, one past and one FUTURE, the naive fix loses the
    unrelated future-day clash as well, with two swallowed exceptions and
    nothing on any screen. The other possible shape is worse again: a
    refusal DICT would be read as a successful move, because that function
    reads nothing off the result — `moved.append` runs unconditionally.
  ...
  - **The card's clock is read ONCE, above the row loop — and the first cut
    read it per row** (`night_has_gone` opens a connection of its own, so a
    seven-row card cost seven; measured 7 → 1). Cheap either way — a Save
    tap, not a render payload — but it is the shape
    `weekly-plan-last-clock-reads` settled on, and it buys the property
    that every row is judged against the SAME day rather than one being
    refused because midnight passed between two iterations.
    **It is the JUDGEMENT that is one day, not the whole card**, and saying
    otherwise was the first wording: `apply_pick`'s backstop reads the
    clock again further down and can refuse a row this loop accepted. That
    is not wrong — by then the night really is over — but it surfaced a
    real hole the hoist's own test found: **that raise propagated out of
    `apply_proposal` and took every remaining row of the card with it**,
    which is the one thing the per-row refusal exists to prevent. Caught
    per row now, through `_as_why`, so the card says one thing about one
    fact however the refusal arrived.
  ...
  - **THE SET IS INCOMPLETE, AND THAT IS NOT THE SAME AS "OUT OF SCOPE".**
    `resolve_open_slot` reaches `_replace_slot_entries` DIRECTLY, so it
    bypasses all five doors above, and `apply_pick`'s backstop cannot catch
    it — it is an existing door that never passes through `apply_pick`.
    Reproduced here on an approved week: settling a past open night wrote
    the meal and put **`{'Carrots': '3'}`** on the shopping list for a
    dinner that was over, which is the same exposure used above to justify
    guarding "Swap · I'll pick". It is left because it carries a genuine
    product question that is Emily's — should settling a past open slot be
    refused at all, or is answering an old question a repair? — and it has
    its own card. One piece of good news, measured: the Now needs-you card
    resolves through the same route but is only built `for offset in (0,
    1)`, so Now can never name a past date; the exposure is the Plan screen
    plus a direct POST.
  - **`swap_component_in_plan` is NOT the same gap, and an earlier version
    of this entry named it as one.** It takes `(weekly_plan_id,
    component_category, old_meal, new_meal, food_groups)` — **no date at
    all** — because a component plan's rows are keyed by category rather
    than by day, so "refuse a past night" has no target there as stated.
    Something is still missing for those households; it is a different
    question and needs a different answer.
  - **THE MODEL IS TOLD THE SERVER'S DATE WHILE THIS REFUSES ON THE
    HOUSEHOLD'S — named, not fixed.** `agent.py`'s per-turn block is
    `today = datetime.date.today()`, interpolated as "Today's date is … Use
    this to resolve relative dates like 'today'". The chat door is the one
    fully reachable door here, so the two clocks matter. **Direction is
    what keeps it off the live list:** in production the UTC server is
    always at or ahead of a Toronto household, so a date resolved against
    it can only ever MISS a refusal, never invent one. But
    `households.timezone` is settable, and for a household EAST of the
    server the model is told the household's YESTERDAY — so "change
    tonight's dinner" resolves to a night this branch then refuses, and the
    app declines tonight. That is precisely the state
    `test_a_household_a_day_ahead_is_refused_the_servers_today` pins as
    CORRECT behaviour, reached from the prompt rather than from a stale
    screen. The false-positive analysis below names only the
    browser-vs-stored-zone split; this is a second one,
    prompt-vs-stored-zone, and the honest fix for both is the stored zone.
  - **MERGE ORDER, AND GIT WILL NOT WARN ANYBODY.**
    `overnight/drop-dish-refuses-the-past` (pushed, not yet on main)
    hardcodes the same sentence with its own inline `if meal_date < today:`
    and a duplicated copy of the clock reasoning, in a DIFFERENT region of
    this same file. Checked here rather than taken on trust: `git
    merge-tree` produces **no conflict markers in `app/tools/weekly_plan.py`
    at all** — only CLAUDE.md conflicts, because both branches add an entry
    at the top of this log, and resolving that says nothing about the code.
    So merging both leaves a fourth hardcoded copy of the one sentence,
    which is exactly what `NIGHT_GONE`'s own comment exists to prevent. **Whoever merges
    second should repoint `drop_dish_from_day` at `night_has_gone` /
    `NIGHT_GONE`**, the way `add_dish_day` is repointed here.
  ...
  - `tests/test_swap_refuses_the_past.py` (35). **26 red against the
    unmodified `app/`, 9 green — and the red count is worth much less than
    it looks.** 12 never reach an assertion at all (`AttributeError:
    swap_meal_in_plan_for_chat`, the only kind of red a test of a new
    function can have). Of the 14 that do, **7 fail on the assertion they
    are named after and 7 fail earlier** — on a picker guard, a
    precondition, or a missing-key 500 — every one for the right underlying
    reason (the door did not refuse) but none of them evidence for its own
    specific claim. All of them say which they are in their own docstrings.
    **THE FIRST VERSION OF THIS BULLET SAID 23 AND 11**, stably off by one
    in both figures against a reviewer's five independent pins, in an entry
    that makes a point of precision about exactly this statistic.
    **TWELVE mutations are the real evidence and every one bit**: `<=` for
    `<` (7 red), the server's clock (3), the naive in-function fix (4 — all
    three generation tests), a connection held across the clock read (1,
    `assert 2 == 1`), dropping the `apply_pick` backstop (2), dropping the
    change card's row check (4), the fragment given a capital and a stop
    (2), the twin reading positional args only (1), this file's own `_day`
    repointed at the process's clock (0 in Toronto, **5 under a real
    straddle** — the harness trap, reproduced), un-hoisting the card's
    clock (1), dropping the `apply_pick`-refusal catch (1), and the
    blocker's own fix removed (1 red at saturday AND sunday).
  - **Four existing files were re-seeded honestly, each with a note saying
    ...
    class, same presentation: the app is right and the seed is wrong. The
    re-seed is sound in the way that matters: those files pass **112/112
    against the UNGUARDED app**, which is the symmetry that says the
    harness followed the app rather than being bent to fit it.
  - **THE RE-SEED BROKE THE `clock (saturday)` MATRIX, AND THE NOTE THAT
    SAID IT COULDN'T IS THE LESSON.** That note read "nothing in this file
    asserts a weekday" — true as written, and beside the point:
    `swap_in_place._minutes_cap` ends `cap if (cap and weekday < 5) else
    None`, so `test_a_weeknight_limit_still_applies_to_an_untagged_night`
    asserts THE RESULT OF A WEEKDAY BRANCH without naming a weekday. The
    old seed pinned this calendar week's Monday and so held on whatever day
    the suite ran; `DAY1 = household_today()` is Saturday or Sunday twice a
    week. Measured: `clock (saturday)` went **36 passed on main to 1 failed
    / 35 passed**, and `clock (sunday)` 0 → 1 on top of its own pre-existing
    failure — on a matrix that runs on every PR and every push to main and
    whose entire stated purpose is catching weekday cliffs. Fixed by
    `WEEKNIGHT`, the first Monday-to-Friday day of the seeded week (any
    seven consecutive days hold five, so it always exists, is never behind
    today, and is always inside the period). All four pins are 36/36 now,
    matching main. **When a seed moves from a fixed weekday to "today",
    sweep for what BRANCHES on the weekday, not for what asserts one** —
    and run all four pins, which is the only thing that would have caught
    it. Its two neighbours in the same class are safe because `rush` and
    `unrushed` both return before that line ever runs. Swept afterwards:
    `app/` holds exactly TWO places that branch on a MEAL DATE's weekday —
    this one and `agent.py`'s own weeknight cap in the plate pass — and
    every other `.weekday()` in it is deriving a week start or parsing a
    calendar rule.
  - **`_plain_plan` was left holding TWO clocks, and it only shows in the
    straddle job.** It took `day=ADD_SRC` (the household's today) while
    still filing the plan at `create_weekly_plan(_monday())`, and
    `_monday()` reads the SERVER's. Those land in different weeks whenever
    the two clocks disagree across a Monday, and `plan_meal`'s own period
    guard then refuses outright — reproduced on the merge base with a day
    outside that week: "2026-09-23 isn't in weekly plan 1's period". Both
    reachable pairs are the straddle job's own zones, and one of them is
    **production's own direction every Sunday evening** (server Monday,
    household Sunday). It is `create_weekly_plan(day)` now, which is
    byte-identical for every other caller since `MON` is `_monday() + 0`
    and immune by construction rather than by arithmetic.
  - **Numbers, measured, and ALL FOUR WEEKDAY PINS this time — which is
    what the blocker above was about.** `TZ=America/Toronto` throughout.
    Live (a Thursday) **5453 passed / 0 failed**, +35 on the 5418 baseline,
    exactly the new file and nothing deleted or weakened.
    `clock (monday)` **5450 / 0**, `clock (friday)` **5450 / 0**,
    `clock (saturday)` **5450 / 0** — 1 failed before the fix — and
    `clock (sunday)` **5449 / 1**, that one being
    `test_tonight_night_off.py::test_the_night_is_planned_empty_and_never_open`,
    **confirmed pre-existing by running it at the same pin against the
    merge base's own app and tests** rather than assumed from the fact
    that this branch does not touch the file. (3 skipped under every pin:
    the `live_clock` photo tests.)
  - **THE STRADDLE, AND THE DIRECTION LABEL THIS ENTRY GOT BACKWARDS.**
    Checked before it was quoted, which is the correction the 2026-09-16
    entry demands: at the hour these ran, `Pacific/Niue`, `Asia/Tokyo` and
    `Etc/GMT+12` were all on Toronto's date and would have exercised
    nothing. `Pacific/Kiritimati` (UTC+14) did straddle, verified at both
    ends of the run: **process 2026-09-18, household 2026-09-17 — the
    PROCESS AHEAD of the household, which is production's own direction**
    (a UTC container serving a Toronto house). The first version of this
    bullet wrote that pair round the other way and so claimed the
    non-production direction while having measured the production one —
    the same mistake, one layer down, that the entry it cites exists to
    prevent. **3 failed / 5450 passed here against 3 failed / 5415 on the
    merge base at the same zone, the same three tests byte for byte** — the
    pre-existing `test_frozen_clock.py` seam east of UTC+9 — so this adds
    no straddle failure, and the +35 is the new file.
- **2026-09-17 — A `--today` pin means the same thing east of UTC+9 now, so the
  straddle job covers the whole day. Branch
  `overnight/today-pin-east-of-utc9`, NOT merged at the time of writing.**
  ...
  - **The mechanism, measured rather than guessed.** `_freeze_args` reads a pin
    as LOCAL wall time and freezes at the UTC instant behind it
    (`local - offset`), which is right. At the default 09:00 that instant is
    `09:00 - offset`, and for any offset above +9 it lands on the PREVIOUS
    calendar day: Kiritimati (+14) pins `2026-09-13` at `2026-09-12 19:00 UTC`.
    SQLite's `'now'`, node's `toISOString()` and every
    `DEFAULT (datetime('now'))` column all answer in UTC, so they honestly read
    the 12th while `date.today()` reads the 13th. **Nothing in the machinery was
    wrong** — every clock is consistent with one instant. What was wrong was what
    the three guards ASSERTED: that a UTC read carries the pin's LOCAL date,
    which is only true at or west of UTC+9.
  - **THE DEFAULT HOUR IS NOT THE FIX AND CANNOT BE, and this is arithmetic
    rather than taste.** For the local and UTC dates to coincide, the pinned
    hour H must satisfy `0 <= H - offset < 24` for every offset in -12..+14 —
    i.e. `H >= 14` and `H < 12`. There is no such H. Reading the pin as UTC
    instead mirrors the problem (`H >= 12` and `H < 10`). So 09:00 stays, for
    the reason it was picked: it is inside every window the app reasons about.
    A mutation moving it to 14:00 reddens four tests.
  - **The guards measure against PYTHON'S FROZEN CLOCK now, not against
    `ISO_PIN`** — `conftest.pinned_utc_now()`, one documented helper, three
    callers. That is the claim they are named for ("both clocks or neither") and
    it is strictly STRONGER than the date prefix it replaces, because it pins
  ...
  - **CI: Kiritimati is ADDED, not swapped for Tokyo — and this is EMILY'S
    ...
    **SOMEBODY WITH REPO SETTINGS MUST ADD `straddle (Pacific/Kiritimati)` to
    ...
    2026-09-15 entry below, one zone longer.
  - **THE AWARE-DATETIME SEAM IS NOT CLOSED, and east of +9 it is worse.**
    freezegun applies `tz_offset` on top of an already-tz-aware conversion, so
    under a pin `datetime.now(timezone.utc)` hands back the local wall time
    wearing a UTC label. Under Niue that is eleven hours out on the right day;
    under Kiritimati it is fourteen hours out and **on the wrong DATE**
    (09-13 09:00+00:00 where the honest answer is 09-12 19:00+00:00), which is
    the escalation worth knowing. Left open deliberately: the only two fixes are
    ...
    **"Wrong together and agree" IS TRUE OF THE DATES ONLY, AND THAT
    ...
    adding Kiritimati there does not touch this seam.**
  - **Numbers, whole suite, 2026-09-17, and which runs were REAL straddles.**
    `5419 passed / 0 failed` at `TZ=America/Toronto`, `UTC`, `Asia/Tokyo`,
    `Etc/GMT-13`, `Pacific/Kiritimati` and `Pacific/Niue` (5418 before; +1 is
    the new test exactly, no test deleted or weakened). **Two of those were
    checked to be straddling at the instant they ran, start AND end, rather
    than assumed from the zone** — the trap this log has recorded twice:
    Kiritimati (process 2026-09-18 against Toronto 2026-09-17, i.e. the
    production DIRECTION and the hours the job could not see before) and Niue
    (process 2026-09-16 against Toronto 2026-09-17, the other direction).
    Tokyo and `Etc/GMT-13` ran at Toronto 05:26 and 05:29, when both were on
    Toronto's date — green, but not straddle evidence, and said so here rather
    than quoted as if it were. The four weekday pins at `TZ=America/Toronto`:
    monday, friday and saturday `5416 passed + 3 skipped / 0 failed` (the 3 are
    the `live_clock` photo tests), sunday `1 failed` — pre-existing, see below.
  - **TWO PRE-EXISTING FAILURES FOUND ON THE WAY, both reproduced on the merge
    base with this branch stashed, NEITHER FIXED — each its own card.**
    - `test_tonight_night_off.py::test_the_night_is_planned_empty_and_never_open`
      fails under a SUNDAY pin only, on `main` as well. Measured, not
      reasoned: the `dinner_decision` it objects to is for **2026-09-21,
      "Tomorrow needs a dinner"** — a day outside the planned week — and not
      for the Wednesday night the test just took off. `TONIGHT = WED` is
      module-level, so on the last day of the planned week tomorrow falls off
      the end and the band legitimately asks about it. The assertion is too
      broad: it wants "no dinner_decision FOR TONIGHT", not "none at all".
    - A PINNED run east of UTC+9 fails four tests around
      `get_already_have_decisions` (`test_grocery_have_it_and_instead.py` x2,
      `test_tools.py` x2) — measured byte-identical on the merge base at
      `TZ=Pacific/Kiritimati` with a monday pin. That function computes its
      cutoff from `date.today()` (the PROCESS's local date) and compares it
      against `removed_at`, which SQLite stamps in **UTC**; east of +9 those
      are different days, and a MONDAY pin puts the cutoff exactly on the
      boundary so a row removed "now" falls just outside it. Latent in
  ...
  - `tests/test_frozen_clock.py` grew one test,
    ...
    every month) and asserts all four clocks against the one instant. It is what
    stops the old assertion coming back from either side.
  - **Five mutations run, every one bites**, at `TZ=Pacific/Kiritimati`:
    `_sqlite_now` to local (4 red), the default hour to 14:00 (4 red),
...
- **2026-09-15 — The suite is timezone-independent now, and CI has a
  ...
  - **CI: the Toronto pin STAYS on both existing jobs, and a third job
    ...
    **CLOSED 2026-09-17 on `overnight/today-pin-east-of-utc9` — the gap is
    gone, and the advice in the sentence above was NOT taken.** Kiritimati is
    ADDED as a third zone rather than swapped in for Tokyo, because removing a
    matrix entry removes its CHECK NAME and would strand branch protection on
    a check that can never report; a third name,
    `straddle (Pacific/Kiritimati)`, now also needs adding there. See that
