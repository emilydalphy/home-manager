...
def test_the_clock_is_read_once_for_the_whole_screen(monkeypatch):
    """
    GUARD on the cost, not on the behaviour. household_now opens its own
    connection, so it is resolved ONCE at the entry point and threaded
    down to moves_for_day and featured_move_id — never per move, never
    inside an open write transaction (see the "database is locked"
