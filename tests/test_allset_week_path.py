...
"""
    ...
    assert out["unloaded"]["tabs"] == ["week"] and out["unloaded"]["steps"] == [], "no week loaded: the Plan root"
    assert out["draft"]["tabs"] == ["week", "week"] and out["draft"]["steps"] == [], "a draft's root is its review"
    assert out["set"]["tabs"] == ["week", "week", "week"] and out["set"]["steps"] == ["review"], (
        "an approved week: Check the week, the day cards"
    )
