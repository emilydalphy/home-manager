...
def test_the_write_lock_is_held_from_the_first_read(approved_week):
    """
    GUARD, pinned by mutation: drop the explicit BEGIN IMMEDIATE and this
    fails, because sqlite3's default opens the transaction at the first
    write rather than at the first read.
    """
    pid, tg = approved_week
...
def test_two_adults_changing_the_same_answer_leave_one_row(approved_week):
    """
    GUARD, pinned by mutation: without BEGIN IMMEDIATE both writers can read
    the pre-tap slot and two `open` rows land on one day — audit_plan_slots'
    `duplicated`, which this repo's own notes call "how a night nobody is
    home ends up with groceries bought for it".
    """
    pid, tg = approved_week
    ...
    def _go():
        ...
        try:
            hol._reopen(pid, tg, "Thanksgiving")
        except Exception as exc:  # a loser is allowed to lose loudly
            errors.append(exc)
    ...
    rows = _rows(pid, tg)
    assert len(rows) == 1, f"{len(rows)} rows on one slot; errors={errors}"
...
def _count_connections(monkeypatch):
    """Count get_conn calls made inside _reopen, wherever the name was bound."""
    import app.db as db
    seen = {"n": 0}
    real = db.get_conn
