...
class TestABrandNewHouseholdWithNoPlanAtAll:
    ...
    def test_the_card_is_offered_and_the_meal_is_saved(self):
        _a_recipe()
        # The holiday ask is subtracted, and everything else still has to be
        # exactly what it always was. An exact list made this red for about
        # four days around each of the seven asking holidays — a feature
        # reported as a regression — but relaxing it to `in` would also pass
        # with five unrelated cards beside the dinner, which is not what this
        # ever claimed. Reproduced pinned to Thanksgiving (2026-10-12),
        # Christmas and New Year's Day; nothing else in this file reads it.
        offered = [i["type"] for i in tools.get_needs_you_items()]
        assert [t for t in offered if t != "holiday_ask"] == ["dinner_decision"], offered
