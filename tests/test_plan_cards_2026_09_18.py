...
def test_the_help_button_degrades_to_the_ask_sheet_until_the_help_sheet_lands():
    src = _extract("openWeekHelp", SHELL_JS)
    assert "window.openHelpSheet" in src and "screenName" in src and "openAskSheet()" in src
...
def test_the_cook_ahead_ask_is_gone_from_all_set_and_the_root():
    for gone in ("cookAheadAskCardHtml", "cookAheadAskState", "ensureCookAheadAskItems", "submitCookAheadAsk",
        ...
        assert gone not in SHELL_JS, gone
    # Cook's own link still has somewhere to land until its branch removes it.
    assert "function openCookAheadAskFromCook()" in SHELL_JS
...
def test_the_swap_sheet_shows_the_eyebrow_the_title_three_picks_and_the_two_lines():
    ...
    st = {
        ...
        "options": [
            {"meal_name": "Sheet-Pan Sausages", "reason": "uses the sausages", "minutes": 30},
            {"meal_name": "Chicken fajitas", "reason": "same tortillas", "minutes": 25},
            {"meal_name": "Veggie quesadillas", "reason": "no shopping", "minutes": 15},
        ],
    }
    html = _run(_prelude() + f"weekState.days = {json.dumps(days)};\n"
...
def test_the_swap_sheet_is_a_sheet_and_the_picks_go_through_the_new_routes():
    ...
    assert "'/swap-options?entry_id='" in SHELL_JS and "'/swap-pick'" in SHELL_JS
