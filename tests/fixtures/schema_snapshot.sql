-- A SNAPSHOT of app/schema.sql as it stood on 2026-09-08 (commit 7c88e19d),
-- pinned so the upgrade path can be tested. Not a copy of today's schema —
-- deliberately an OLD one.
--
-- Loop Board "A column added to schema.sql without a migration is missing
-- on every database that already exists" (2026-09-12).
--
-- WHY THIS FILE EXISTS. A database that already exists — Emily's on
-- Railway, the beta tester's — is never rebuilt from schema.sql. Startup
-- runs schema.sql (whose every CREATE is IF NOT EXISTS, so it adds new
-- TABLES and nothing else) and then app/db.py's _MIGRATIONS, which is the
-- only thing that can add a COLUMN to a table that is already there. So a
-- column added to schema.sql and forgotten in _MIGRATIONS exists on every
-- fresh database — every test run, every new checkout — and on no real
-- one. Every test passes and the live app raises "no such column".
--
-- The fault is invisible in the "after": a fresh database has the column
-- either way. Catching it needs a database built BEFORE the column
-- existed. This file is that database.
--
-- WHY THIS DATE. It is the oldest schema.sql reachable in the working
-- clone (the checkout is shallow, back to 2026-09-08). Older is strictly
-- better here: the further back the snapshot, the more real migrations the
-- guard exercises. Every schema revision from this one forward was checked
-- on 2026-09-12 and every one upgrades cleanly — there is no existing
-- drift, so pinning the oldest costs nothing and tests the most.
--
-- WHEN TO UPDATE IT. Rarely, and deliberately. NEVER to make a red test go
-- green — a red test here is the guard doing its job, and the fix is
-- almost always to add the missing entry to _MIGRATIONS. Moving it forward
-- only ever tests LESS. Refresh it only if you genuinely want to stop
-- supporting an upgrade from this point, and say in the commit why.
